...
void Model::render(mat4 modelMatrix, mat4 viewMatrix, mat4 projMatrix, LightsInfo lights, GLuint mode)
{
...
void Model::load_coords(vec3* verteces, size_t count)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, count * sizeof(vec3), verteces, GL_STATIC_DRAW);
...
void Model::load_texcoord(vec2* tex, size_t count)
{
    GLuint tex_vbo = 0;
    glGenBuffers(1, &tex_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, tex_vbo);
    glBufferData(GL_ARRAY_BUFFER, count * sizeof(vec2), tex, GL_STATIC_DRAW);
...
void Model::load_texture(string path)
{
...
void Model::setMaterial(vec3 a, vec3 d, vec3 s, GLfloat shine)
{
    material.ka = a;
    material.kd = d;
    material.ks = s;
